  ...
  return (
    <Marco>
      ...
      <div className="mb-5 flex flex-wrap items-center justify-between gap-3">
        <h1 className="text-xl font-medium text-neutral-100">Ingresos</h1>
        <div className="flex items-center gap-2">
          <button onClick={() => setMesRef(sumarMes(mesRef, -1))} className="rounded-lg border border-neutral-800 bg-neutral-900 text-neutral-100 placeholder-neutral-500 px-3 py-1.5 text-sm">←</button>
      ...
      <Bloque titulo="Ingresos por categoría">
        {stats.cats.length === 0 && <p className="text-sm text-neutral-500">Sin datos este mes.</p>}
      ...
      <Bloque titulo="Ingresos por profesional">
        {stats.profs.length === 0 && <p className="text-sm text-neutral-500">Sin datos este mes.</p>}
...
function Kpi({ label, valor, destacado }) {
  return (
    <div className={`rounded-lg p-3 ${destacado ? 'bg-[#c9a227]/10' : 'bg-white/5'}`}>
      <p className="text-xs text-neutral-400">{label}</p>
      <p className={`text-lg font-medium ${destacado ? 'text-[#e3b23c]' : 'text-neutral-100'}`}>{valor}</p>
    </div>
  )
}
function Bloque({ titulo, children }) {
  return (
    <div className="mb-6">
      <p className="mb-3 text-sm font-medium text-neutral-300">{titulo}</p>
  ...
  return (
    <div>
      <div className="mb-1 flex justify-between text-sm">
        ...
        <span className="font-medium text-neutral-100">{valor}</span>
      </div>
      <div className="h-2 w-full overflow-hidden rounded-full bg-neutral-800">
        <div className="h-full rounded-full bg-[#e3b23c]" style={{ width: `${Math.max(4, pct)}%` }} />
      </div>
    </div>
  )
}
